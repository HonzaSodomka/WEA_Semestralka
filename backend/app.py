...
from database.rating_operations import get_user_rating, add_or_update_rating
...
def rate_book(isbn):
    ...
    success, message = add_or_update_rating(isbn, user_id, rating_value)
    
    if success:
        info_logger.info('User %s rated book %s with %d stars', user_id, isbn, rating_value)
        return jsonify({'message': message}), 200
    else:
        info_logger.warning('Failed to rate book %s: %s', isbn, message)
        return jsonify({'error': message}), 400
...
def get_rating(isbn):
    ...
    rating = get_user_rating(isbn, user_id)
